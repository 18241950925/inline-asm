...
从 Bank 5 固定模上下文表中选择并激活表项。`mod_id` 为 8-bit，范围 0..255，编码在原始指令 `[21:14]`，对应 26-bit HPU 命令的 `OP2_8[14:7]`；其余操作数字段必须为 0。
...
模表由 `dload type=2` 预先安装；custom1 中的对象号只作为 DMA 传输句柄，不是 `pmodld` 的源操作数。旧语法 `pmodld psrc, idx1, cfg` 不再接受。
